...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		module.exports = factory(require("fs"), require("websocket"), require("wrtc"));
	else if(typeof define === 'function' && define.amd)
		define("sp", ["fs", "websocket", "wrtc"], factory);
	else if(typeof exports === 'object')
		exports["sp"] = factory(require("fs"), require("websocket"), require("wrtc"));
	else
		root["sp"] = factory(root["fs"], root["websocket"], root["wrtc"]);
...
if (typeof window === "undefined")
	...
	CallbackBuffer = __webpack_require__(3);
	}
else
	{
	Logger = window.sp.Logger;
	CallbackBuffer = window.sp.CallbackBuffer;
	}
...
var logger = new Logger("RpcCommunicator", "splogs");
...
var sendResponse = function(err, result, id, connectionId)
	...
	try	{
		if (err)
			...
			logger.log("Exception in executing a RPC method: " + code + " RpcCommunicator::onMessage() >> " + path + " " + msge);
			}
		else
			{
...
var handleReturnValue = function(message)
	...
	try	{
		...
		if (message.id)
			...
			logger.log("RpcCommunicator::handleReturnValue() error: " + JSON.stringify(error));
		}
	catch (e)
		{
		logger.log(e);
...
self.exposeRpcMethod = function(name, object_, method_)
	{
	//logger.log("RpcCommunicator::exposeRpcMethod name: " + name + ", object_: " + object_ + ", method_: " + method_);
	try	{
...
self.callRpc = function(method, params, object, listener, connectionId)
	...
	try	{
		if (typeof listener == "function")	// call: expects a response object
			...
			//logger.log("RpcCommunicator::callRpc() pushed back callback");
			...
			//logger.log("RpcCommunicator::callRpc() sendMessage returned");
			callSequence++;
			}
		else
...
self.notifyAll =  function(method, params)
	...
	try	{
		...
		//logger.log("RpcCommunicator::notifyAll() sending messages completed");
		}
	catch(e)
		{
		logger.log(e);
...
self.onMessage = function(messageData, connection)
	{
	//logger.log("RpcCommunicator::onMessage() " + messageData);
	//logger.log(typeof messageData);
	try	{
		var pipeTarget = connection.getPipedTo();

		if (pipeTarget != null)
			{
			//logger.log("RPCCommunicator::onMessage() relaying a piped message");
		...
		if (messageData instanceof ArrayBuffer)
			{
			//logger.log("RPCCommunicator::onMessage() received arraybuffer");
...
self.addConnection = function(conn)
	{
	try	{
		//logger.log("RpcCommunicator::addConnection, connectionid was " + conn.getId());
...
if (typeof window === "undefined")
	...
	Logger = __webpack_require__(0);
	WebSocket = __webpack_require__(14);
	}
else
	{
	Logger = window.sp.Logger;
...
var logger = new Logger("WebSocketConnection", "splogs");
...
function DataChannelConnection(dataChannel_)
{
var self = this;
...
var dataChannel = dataChannel_;

var logger = new Logger("DataChannelConnection", "splogs");
...
self.close = function()
	{
	logger.log("DataChannelConnection::close");
	};

self.sendBinary = function(data)
	{
	//logger.log("DataChannelConnection::sendBinary() " + typeof data);

	try	{
		if (dataChannel.readyState == "open")
			{
			//logger.log("DataChannelConnection::sendBinary() readyState was open");
...
if (typeof window === "undefined")
	{
	Logger = __webpack_require__(0);
	WebRtcPeerConnection = __webpack_require__(7);
	}
else
	{
	Logger = window.sp.Logger;
	WebRtcPeerConnection = window.sp.WebRtcPeerConnection;
	}

var logger = new Logger("WebRtcConnector", "splogs");
...
if (typeof window === "undefined")
	{
	Logger = __webpack_require__(0);
	DataChannelConnection = __webpack_require__(5);

	var webrtclib = __webpack_require__(15);
	...
	{
	Logger = window.sp.Logger;
	DataChannelConnection = window.sp.DataChannelConnection;
	}
...
var logger = new Logger("WebRtcPeerConnection", "splogs");
...
self.createDataChannelConnection = function(callback)
	...
	channel.onopen = function()
		{
		logger.log("WebRtcPeerConnection::channel.onopen");
...
var onIceCandidate = function(e)
	...
	logger.log("iceListener was " + iceListener);
...
self.onConnectionAnswerReceived = function(descriptor)
	...
	peerConnection.setRemoteDescription(new RTCSessionDescription(descriptor),
		function()
			{
...
self.onConnectionOfferReceived = function(descriptor, connectionId, callback)
	...
	peerConnection.setRemoteDescription(desc,
		...
		function(err)
			{
			logger.log("WebRtcPeerConnectiononConnectionOfferReceived setting remote description failed " + err);
			});
	};
...
self.onIceCandidateReceived = function(iceCandidate)
	...
	peerConnection.addIceCandidate(new RTCIceCandidate(iceCandidate),
		...
		function(err)
			{
			logger.log("WebRtcPeerConnectiononIceCandidateReceived adding Ice candidate failed "+err);
			});
	};
...
if (typeof window === "undefined")
	{
	Logger = __webpack_require__(0);
	RpcCommunicator = __webpack_require__(1);
	WebSocketConnection = __webpack_require__(2);
	Client = __webpack_require__(4);
	WebRtcConnector = __webpack_require__(6);
	}
else
	{
	Logger = window.sp.Logger;
	RpcCommunicator = window.sp.RpcCommunicator;
	WebSocketConnection = window.sp.WebSocketConnection;
	Client = window.sp.Client;
	WebRtcConnector = window.sp.WebRtcConnector;
	}

var logger = new Logger("CommunicationClient", "splogs");
...
/**
* Calls an JSON-RPC method on a remote client identified by clientId. The fastest available connection is
...
self.callRpcOnClient = function(clientId, methodName, params, obj, callback)
	...
	if (clients.hasOwnProperty(clientId))
		...
		if (clients[clientId].isWebRtc())
			{
			logger.log("CommunicationClient::callRpcOnClient - Calling client RPC through WebRtc, preferredConnectionId was: " + clients[clientId].getPreferredConnectionId() + "\n");
		...
		//call though a local hub
			...
			{
			logger.log("CommunicationClient::callRpcOnClient - Calling client RPC through local hub" + "\n");
			...
			logger.log("CommunicationClient::callRpcOnClient - Notifying client RPC through the server, serverId: " + serverId + "\n");
...
self.callRpcOnConnection = function(connectionId, methodName, params, obj, callback)
	...
	logger.log("CommunicationClient::callRpcOnConnection - Making a RPC call over direct connection" + "\n");
...
self.notifyClient = function(clientId, methodName, params)
	...
	if (clients.hasOwnProperty(clientId))
		...
		if (clients[clientId].isWebRtc())
			{
			logger.log("CommunicationClient::notifyClient - Notifying client through WebRtc" + "\n");
		...
		//call though a local hub
			...
			{
			logger.log("CommunicationClient::notifyClient - Notifying client through local hub" + "\n");
			...
			{
			// call through the server
			logger.log("CommunicationClient::notifyClient - Notifying client through the server, serverId: " + serverId + "\n");
...
self.createDirectConnection = function(clientId, callback)
	{
	logger.log("CommunicationClient::createDirectConnection(clientId: " + clientId + ")" + "\n");
	...
	if (clients[clientId].isWebRtc())
		...
		self.createPipe(clientId, callback);
	};
...
self.onWebRtcConnected = function(clientId, connectionId)
	{
	logger.log("CommunicationClient::onWebRtcConnected(clientId: " + clientId + ", connectionId: " + connectionId + ")" + "\n");
...
self.onWebRtcDisconnected = function(partnerId)
	{
	logger.log("CommunicationClient::onWebRtcDisconnected(partnerId: " + partnerId + ")" + "\n");
...
self.onAdditionalDataChannelOpen = function(clientId, connectionId)
	{
	logger.log("CommunicationClient::onAdditionalDataChannelOpen(clientId: " + clientId + ", connectionId: " + connectionId + ")" + "\n");
...
self.createPipe = function(clientId, callback)
	{
	logger.log("CommunicationClient::createPipe(clientId: " + clientId + ")" + "\n");
	...
	pipeConnection.connect(serverConnectionOptions, function()
		...
		logger.log("CommunicationClient::createPipe - Pipe connected to the WebSocket server, pipeId: " + pipeId + "\n");
...
self.requestPipe = function(targetId, originatorId, connectionId, callback)
	{
	logger.log("CommunicationClient::requestPipe(targetId: " + targetId + ", originatorId: " + originatorId + ", connectionId: " + connectionId + ")" + "\n");
	...
	pipeConnection.connect(serverConnectionOptions, function()
		...
		logger.log("CommunicationClient::requestPipe - Pipe connected to the WebSocket server, pipeId: " + pipeId + "\n");

		communicator.callRpc("registerAsPipe", [targetId], self, function(err, data)
			{
			logger.log("CommunicationClient::requestPipe - registerAsPipe returned" + "\n");
...
self.pipeToExternalConnection = function(connectionId, externalConnection)
	...
	logger.log("CommunicationClient::pipeToExternalConnection(), connectionId: " + connectionId + ", externalId: " + extId + "\n");
...
var addClient = function(clientId, clientType, arrivedBeforeUs)
	{
	logger.log("CommunicationClient::addClient(clientId: " + clientId + ", clientType: " + clientType + ", arrivedBeforeUs: " + arrivedBeforeUs + ")" + "\n");
...
var updateConnectedClients = function(hubId, callback)
	{
	logger.log("CommunicationClient::updateConnectedClients(hubId: " + hubId + ")" + "\n");
	...
	communicator.callRpc("getConnectedClients", [groupId], self, function(err, connectedClients)
		{
		logger.log("CommunicationClient::updateConnectedClients got answer from hub: " + JSON.stringify(connectedClients) + "\n");
...
var connectToLocalHubs = function(hubs, callback)
	{
	logger.log("CommunicationCLient::connectToLocalHubs(hubs: " + JSON.stringify(hubs) + ")" + "\n");
	...
	for (var i in hubs)
		...
		for (var j = 0; j < hubs[i].localIps.length; j++)
			...
			connection.connect(opts, function()
				{
				logger.log("CommunicationClient::connectToLocalHubs - Connected to local hub at ip: " + ip + " port: " + port + "\n");
...
var updateLocalHubs = function(callback)
	{
	logger.log("CommunicationClient::updateLocalHubs() - Getting list of local hubs from the server" + "\n");

	communicator.callRpc("getLocalHubs", [], self, function(err, hubs)
		{
		logger.log("CommunicationClient::updateLocalHubs - Following local hubs are available: " + JSON.stringify(hubs) + "\n");
...
self.connectWithOptions = function(options, clientType_, groupId_, callback)
	{
	logger.log("CommunicationClient::connectWithOptions(options: " + JSON.stringify(options) + ", clientType: " + clientType_ + ", groupId: " + groupId_ + ")" + "\n");
	...
	serverConnection.connect(serverConnectionOptions, function()
		...
		logger.log("CommunicationClient::connectWithOptions - Connected to the WebSocket server, serverId: " + serverId + "\n");

		rtcConnector = new WebRtcConnector(communicator, serverId, (typeof LoaderUtil != "undefined" ? LoaderUtil.WEBRTC_CONFIG : WEBRTC_CONFIG));
		...
		communicator.callRpc("registerAsClient", [null, clientType, groupId], self, function(err, givenId)
			{
			logger.log("CommunicationClient::connectWithOptions - Server replied to registerAsClient call, ownId: " + givenId + "\n");
...
self.onClientConnected = function(clientData, connectionId, callback)
	{
	logger.log("CommunicationClient::onClientConnected(clientData: " + JSON.stringify(clientData) + ", connectionId: " + connectionId + ") - ownId: " + ownId + "\n");
	...
	if (!clients.hasOwnProperty(clientData.clientId))
		{
		logger.log("CommunicationClient::onClientConnected - addClient clientId: " + clientData.clientId + "\n");
...
self.onClientDisconnected = function(clientData, connectionId, callback)
	{
	logger.log("CommunicationClient::onClientDisconnected(connectionId: " + connectionId + ", clientData: " + JSON.stringify(clientData) + ")" + "\n");
...
self.onHubConnected = function(hubData, connectionId, callback)
	{
	logger.log("CommunicationClient::onHubConnected(hubData: " + JSON.stringify(hubData) + ")" + "\n");
	};

self.onHubDisconnected = function(hubData, connectionId, callback)
	{
	logger.log("CommunicationClient::onHubDisconnected(hubData: " + JSON.stringify(hubData) + ")" + "\n");
	};
...
var logger = new Logger("WebSocketRpcConnection", "splogs");
...
self.connect = function(options, callback)
	{
	logger.log("WebSocketRpcConnection::connect()");

	connection.connect(options, function()
		{
		logger.log("WebsocketRpcConnection Connected");
		//logger.log("Creating RPCCommunicator for the Websocket");

		communicator.addConnection(connection);

		logger.log("WebsocketRpcConnection added to communicator");
		callback(null, null);
		});
	};
...
Client: __webpack_require__(4),
WebRtcConnector: __webpack_require__(6),
CommunicationClient: __webpack_require__(8),
RpcCommunicator: __webpack_require__(1),
WebRtcPeerConnection: __webpack_require__(7),
...
var sp = (typeof window === "undefined" ? require("../libs/spaceifyconnect.bundle.js") : window.sp);

var logger = new sp.Logger("HttpParser", "spllogs");
...
var logger = new sp.Logger("PiperClient", "spllogs");

var communicationClient = new sp.CommunicationClient();

var tcpTunnels = new Object();

var webSocketTunnels = new Object();

var tunnelReadyListener = null;

var targetId = null;

var binaryListener = null;

var cookies = null;										// ToDo: this is a hack, we need a separate cookie storage at some point!
...
self.createTcpTunnel = function(host, port, listener, callback)
	{
	logger.log("PiperClient::createTcpTunnel(host: " + host + ", port: " + port + ")" + "\n");

	var hostnameAndPort = host + port;

	for (var tunnelId in tcpTunnels)
		{
		if(tcpTunnels[tunnelId].hostnameAndPort == hostnameAndPort)
			{
			tcpTunnels[tunnelId].listener = listener;
			callback(tunnelId);
			return;
			}
		}

	communicationClient.createDirectConnection(targetId, function(tunnelId)
		{
		logger.log("PiperClient::createTcpTunnel - Direct Connection Ready for TCP tunnel" + "\n");

		communicationClient.callRpcOnConnection(tunnelId, "tunnelTcp", [host, port], self, function()
			{
			logger.log("PiperClient::createTcpTunnel - Tunnel pipe ready to host: " + host + ", port: " + port + "\n");

			tcpTunnels[tunnelId] = {listener: listener, hostnameAndPort: hostnameAndPort};
...
self.createWebSocketTunnel = function(options, listener, callback)
	{
	logger.log("PiperClient::createWebSocketTunnel(options: " + JSON.stringify(options) + ")" + "\n");

	communicationClient.createDirectConnection(targetId, function(tunnelId)
		{
		logger.log("PiperClient::createWebSocketTunnel - Pipe ready for WebSocket, tunnelId: " + tunnelId + "\n");

		communicationClient.callRpcOnConnection(tunnelId, "tunnelWebSocket", [options], self, function()
			{
			logger.log("PiperClient::createWebSocketTunnel - WebSocket pipe ready to host: " + options.host + ", port: " + options.port + "\n");

			webSocketTunnels[tunnelId] = listener;
...
self.callClientRpc = function(id, method, params, selfobj, callback)
	{
	communicationClient.callRpcOnConnection(id, method, params, selfobj, callback);
	};
...
self.onBinary = function(data, clientId, connectionId)
	{
	//logger.log("PiperClient::onBinary(clientId: " + clientId + ", connectionId: " + connectionId + ")" + "\n");

	if (tcpTunnels.hasOwnProperty(connectionId))
		{
		tcpTunnels[connectionId].listener(data);
		}
	};
...
self.onClientConnected = function(client)
	...
	if (client)
		{
		logger.log("PiperClient::onClientConnected - " + JSON.stringify(client) + "\n");

		if (client.getClientType() == "piper")
			{
			logger.log("PiperClient::onClientConnected - clientType is 'piper', trying to build a direct connection to it" + "\n");

			communicationClient.createDirectConnection(client.getClientId(), function(pipeId)
				{
				logger.log("PiperClient::onClientConnected - Direct connection ready, pipeId: " + pipeId + "\n");
...
self.upgradeToWebRtc = function(callback)
	...
	communicationClient.upgradeToWebRtc(targetId, function()
		{
		logger.log("PiperClient::upgradeToWebRtc - targetId: " + targetId + " - Completed" + "\n");
		callback();
		});
	};
...
self.connect = function(host, port, isSsl, groupId, callback)
	{
	logger.log("PiperClient::connect(host: " + host + ", port: " + port + ", isSsl: " + isSsl + ", groupId: " + groupId + ")" + "\n");

	tunnelReadyListener = callback;

	communicationClient.setClientListener(self);
	communicationClient.setBinaryListener(self);

	communicationClient.connectWithOptions({host: host, port: port, isSsl: false}, "piperclient", groupId, function(givenId)
		{
		logger.log("PiperClient::connect - Hub Connection succeeded, givenId: " + givenId + "\n");
...
self.testPing = function(callback)
	...
	communicationClient.createDirectConnection(targetId, function(tunnelId)
		{
		ping(0, tunnelId, callback);
		});
	}

var ping = function(index, tunnelId, callback)
	{
	...
	communicationClient.callRpcOnConnection(tunnelId, "hello", [], self, function(err, data)
		{
		totaltime += Date.now() - startTime;
		logger.log("index: " + index + ", time: " + ((Date.now() - startTime) / 1000) + ", length: " + data.length + "\n");

		ping(index, tunnelId, callback);
		});
	}
...
/*var handler = {
	...
	set: function(target, property, value, receiver)
		{
		logger.log("\SET PROXY:", property + "=" + value);
...
var logger = new sp.Logger("SpXMLHttpRequest", "spllogs");
...
// 	use URL credentials flag - Set if either request URL's username is not the empty string or request URL's password is non-null.
	...
	if (!synchronousFlag)
		...
		if (!redirectingFlag)
			emitEvent({type: "loadstart", loaded: 0, total: 0, lengthComputable: false, target: self});
		...
		if (!uploadCompleteFlag && uploadListenerFlag)
			...
			if (typeof self.upload.onloadstart === "function" && !redirectingFlag)
				self.upload.onloadstart({type: "loadstart", loaded: 0, total: request.length, lengthComputable: true, target: self});
			}
...
self.overrideMimeType = function(mime)
	...
	// LISTENERS: Adapted from "https://developer.mozilla.org/en-US/docs/Web/API/EventTarget - Simple implementation of EventTarget"
...
self.removeEventListener = function(type, listener, obj)
	...
	if (xhr)
		...
		{
		if (eventListenerTypes.indexOf(type) === -1 || !(type in eventListeners))
			return;

		if (typeof obj === "boolean")
			options = { capture: obj, passive: false };

		var stack = eventListeners[type];
		...
		if (eventListeners[type].length == 0)
...
self.dispatchEvent = function(event)
	...
	if (xhr)
		...
		var stack = eventListeners[event.type];
...
// ToDo
	...
	if (contentLength && contentLength == bodyBytesReceived)
		...
		var conTyp = (!overridedMimeType ? contentType : overridedMimeType);

		if (self.responseType == XMLHttpRequestResponseType.BLOB)
			{
			if (typeof window !== "undefined")									// Browser
				{
				self.response = new Blob(response.body, {type : (!overridedMimeType ? contentType : overridedMimeType)});
				}
			else																// 	Node.js
				{
				makeResponseArrayBuffer();
				}
			}
		else if (self.responseType == XMLHttpRequestResponseType.ARRAYBUFFER)
			{
			makeResponseArrayBuffer();
			}
		else if (self.responseType == XMLHttpRequestResponseType.DOMSTRING ||
			self.responseType == XMLHttpRequestResponseType.JSON ||
			self.responseType == XMLHttpRequestResponseType.TEXT)
			{
		...
		else if (conTyp == XMLHttpRequestResponseType.DOCUMENT)
			{
			// ToDo: self.responseXML
			}
...
var addBytesToFetched = function(arr)
	...
	emitEvent({type: "progress", loaded: bodyBytesReceived, total: contentLength, lengthComputable: true, target: self});
...
var requestEndOfBody = function()
	...
	if (typeof self.upload.onprogress === "function")
		self.upload.onprogress({type: "progress", loaded: transmittedBytes, total: bytesToTransmit, lengthComputable: true, target: self});
	...
	if (typeof self.upload.onload === "function")
		self.upload.onload({type: "load", loaded: transmittedBytes, total: bytesToTransmit, lengthComputable: true, target: self});
	...
	if (typeof self.upload.onloadend === "function")
		self.upload.onloadend({type: "loadend", loaded: transmittedBytes, total: bytesToTransmit, lengthComputable: true, target: self});
	}
...
// ToDo
	...
	if (plainHeader)
		emitEvent({type: "progress", loaded: 0, total: 0, lengthComputable: true, target: self});
	...
	emitEvent({type: "load", loaded: bodyBytesReceived, total: (contentLength ? contentLength : 0), lengthComputable: true, target: self});

	emitEvent({type: "loadend", loaded: bodyBytesReceived, total: (contentLength ? contentLength : 0), lengthComputable: true, target: self});
	}
...
var changeReadyState = function(newReadyState, repeat/**/)
	...
	if (self.readyState != newReadyState || repeat)
		...
		emitEvent({type: "readystatechange", target: self});
		}
	}
...
var makeResponseArrayBuffer = function()
	{
	var offset = 0;
	self.response = new ArrayBuffer(bodyBytesReceived);
	var ui8a = new Uint8Array(self.response);

	for (var i = 0; i < response.body.length; i++)
		{
		ui8a.set(response.body[i], offset);
		offset += response.body[i].length;
		}
	}

self.isSpXMLHttpRequest = true;
...
SpXMLHttpRequest.OriginalXMLHttpRequest = (function()
	{
	if (typeof window !== "undefined")
		{
		if (typeof window.OriginalXMLHttpRequest === "undefined")
			window.OriginalXMLHttpRequest = window.XMLHttpRequest;

		return window.OriginalXMLHttpRequest;
		}
	else
		return null;
	})();
...
LoaderUtil.prototype.SERVER_ADDRESS = (function()
	...
	if (typeof window !== "undefined")
		...
		// Assume for now that hostname has three parts, e.g. shop.spaceify.net or museum.spaceify.net
...
if(window)
	...
	window.open = function(unique_name, sp_path, name, specs, callback)
		{
		var port, query, sp_host, spe_host, url, opened;
		...
		core.getApplicationURL(unique_name, function(err, appURL)
			...
			url = spe_host + "remote.html/" + network.remakeQueryString(query, {}, {}, "", true);
...
var logger = new sp.Logger("SpaceifyLoader", "spllogs");
...
self.loadPage = function(url, spHost_, speHost_)
	...
	xhr.addEventListener("loadend", function(e)
		{
		//logger.log("SpaceifyLoader::loadPage() content arrived, readyState==" + xhr.readyState);
...
self.getAllElements = function()
	...
	logger.log("SpaceifyLoader::loadAll() :: Number of elements with sp_src: " + sp_src.length + ", spe_src: " + spe_src.length + ", sp_href: " + sp_href.length + ", spe_href: " + spe_href.length + ", sp_bgnd: " + sp_bgnd.length);
	};
...
self.connect = function(host, port, isSsl, groupId, callback)
	{
	logger.log("SpaceifyLoader::connect(host: " + host + ", port: " + port + ")");

	LoaderUtil.piperClient.connect(host, port, isSsl, groupId, function()
		{
		if (connectionListener)
			connectionListener();
...
function getNetworkInfo(callback)
	...
	var xhr = new SpXMLHttpRequest.OriginalXMLHttpRequest();
...
function prepareLoader(hosts)
	...
	if (window.isSpaceifyNetwork)
		...
		spaceifyLoader.connect(LoaderUtil.SERVER_ADDRESS.host, LoaderUtil.SERVER_ADDRESS.port, LoaderUtil.SERVER_ADDRESS.isSsl, LoaderUtil.SERVER_ADDRESS.groupId, function()
			{
			});
		}
	}
